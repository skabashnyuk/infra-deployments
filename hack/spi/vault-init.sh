...
function ensureRootToken() {
	...
	kubectl --kubeconfig=${VAULT_KUBE_CONFIG} create secret generic ${ROOT_TOKEN_NAME} \
...
if ! timeout 100s bash -c "while ! kubectl get applications.argoproj.io -n openshift-gitops -o name | grep -q spi-vault-in-cluster-local; do printf '.'; sleep 5; done"; then
